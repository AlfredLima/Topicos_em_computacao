...
def GWO(low_boundary, upper_boundary, dimension, population, iterations, radius, D, city, city_centers, show):
        ...
        for i in range(0, population):
            for j in range(0, dimension[0]):
                for k in range(0, dimension[1]):
                    ...
                    # Equation (3.5)-part 1
                    D_alpha = abs(C1*Alpha_pos[j][k]-Positions[i][j][k])
                    ...
                    # Equation (3.5)-part 2
                    D_beta = abs(C2*Beta_pos[j][k]-Positions[i][j][k])
                    ...
                    # Equation (3.5)-part 3
                    D_delta = abs(C3*Delta_pos[j][k]-Positions[i][j][k])
        ...
        if (l % 1 == 0):
            ...
            if show:
                run_pygame(create_city_points(city_centers, D),
                           create_base_stations_points(Alpha_pos, radius))
